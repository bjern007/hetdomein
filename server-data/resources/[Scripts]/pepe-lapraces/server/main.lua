...
AddEventHandler('pepe-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            Framework.Functions.ExecuteSql(false, "UPDATE `lapraces` SET `records` = @records WHERE `raceid` = @raceid", {
                ['@raceid'] = RaceData.RaceId,
                ['@records'] = json.encode(Races[RaceData.RaceId].Records),
            })
            TriggerClientEvent('pepe-phone:client:RaceNotify', src, 'You have the WR van '..RaceData.RaceName..' disconnected with a time of: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        TriggerClientEvent('pepe-phone:client:RaceNotify', src, 'You have the WR van '..RaceData.RaceName..' put down with a time of: '..SecondsToClock(BLap)..'!')
    end
...
Framework.Functions.CreateCallback('pepe-lapraces:server:GetTrackData', function(source, cb, RaceId)
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_metadata` WHERE `citizenid` = @citizenid", {
        ['@citizenid'] = Races[RaceId].Creator,
    }, function(result)
...
AddEventHandler('pepe-lapraces:server:LeaveRace', function(RaceData)
    ...
    local creatorsource = Framework.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('pepe-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' has left the race!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
...
AddEventHandler('pepe-lapraces:server:SaveRace', function(RaceData)
    ...
    Framework.Functions.ExecuteSql(false, "INSERT INTO `lapraces` (`name`, `checkpoints`, `creator`, `distance`, `raceid`) VALUES (@name, @checkpoints, @creator, @distance, @raceid)", {
