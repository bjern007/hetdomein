...
StoreRob.Functions.ClosePinpad = function(HasPutin) {
    ...
    $.post('http://pepe-storerobbery/PinpadClose', JSON.stringify({
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('http://pepe-storerobbery/ClickFail', JSON.stringify({}))
                $.post('http://pepe-storerobbery/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('http://pepe-storerobbery/Click', JSON.stringify({}))
                $.post('http://pepe-storerobbery/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
