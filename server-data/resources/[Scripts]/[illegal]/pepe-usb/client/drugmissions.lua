...
AddEventHandler("Drugs:startMission",function(type)
	...
	if numy == 100 then
		TriggerEvent('Framework:Notify', 'Er zijn momenteel geen missies beschikbaar, probeer het later opnieuw.', 'error') 
	else
		CurrentEventNum = num
		TriggerEvent("Drugs:startTheEvent",num,type)
		TriggerEvent('Framework:Notify', "Volg de locatie op je kaart en steel." ..type.. "")
	end
end)
...
AddEventHandler("Drugs:startTheEvent",function(num,typey)
	...
	while not taken and not StopMission do
		...
		if GetDistanceBetweenCoords(loc.Location, GetEntityCoords(PlayerPedId())) < 2.5 then
			DrawText3Ds(loc.Location[1], loc.Location[2], loc.Location[3],"Druk op [E] om de drugs te stelen")
			if IsControlJustPressed(1,38) then					
				Progressbar(15000,"Drugs stelen")
				if GetDistanceBetweenCoords(loc.Location, GetEntityCoords(PlayerPedId())) < 2.5 then
					TriggerEvent('Framework:Notify', "Je hebt de drugs gestolen.") 
					...
					TriggerEvent('Framework:Notify', "Je ging te ver weg van de drugs.", 'error') 
					RemoveBlip(blip)
					Config.MissionPosition[num].InUse = false
					TriggerServerEvent("Drugs:syncMissionData",Config.MissionPosition)
					taken = true
		...
		if StopMission == true then
			TriggerEvent('Framework:Notify', "Je bent dood, drugsmissie voorbij.", 'error') 
			Config.MissionPosition[num].InUse = false
		end
	end
	RemoveBlip(blip)
...
function CreateMissionBlip(location)
	...
	AddTextEntry('MYBLIP', "Drugs missie")
...
RegisterNetEvent("Framework:client:LastStand")
AddEventHandler("Framework:client:LastStand", function(bool)
	if bool then
...
RegisterNetEvent("Framework:client:IsDead")
AddEventHandler("Framework:client:IsDead", function(bool)
	if bool then
		CancelEvent("Drugs:startMission")
		StopMission = true
		RemoveBlip(blip)
...
function AtmHackFirstSuccess(success)
    ...
    if success then
		...
		TriggerEvent('Framework:Notify', "Je hebt de locatie niet kunnen hacken.", 'error') 
...
function AtmHackSecondSuccess(success)
    ...
    TriggerEvent('mhacking:hide')
    if success then
		Framework.Functions.TriggerCallback("Drugs:StartMissionNow",function()
			TriggerEvent('Framework:Notify', "You successfully hacked into the network") 
			Framework.Functions.TriggerCallback("Drugs:Repbij",function() end)
end)
    else
		TriggerEvent('Framework:Notify', "Je hebt de locatie niet kunnen hacken.", 'error') 
	end
...
function Progressbar(duration, label)
	local retval = nil
	Framework.Functions.Progressbar("drugs", label, duration, false, false, {
