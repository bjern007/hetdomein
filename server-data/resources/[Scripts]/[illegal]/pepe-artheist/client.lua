...
Citizen.CreateThread(function()
    while true do
        ...
        if heistDist <= 3.0 then
            sleep = 1
            Framework.Functions.TriggerCallback('pepe-arthheist:server:getCops', function(cops)
                if cops >= 1 then
                ...
                elseif sellDist <= 3.0 then
                    ...
                    Framework.Functions.Notify('Er zijn niet genoeg agenten aanwezig! 1 nodig', 'error')
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            if weapon == GetHashKey('WEAPON_SWITCHBLADE') then
                                if not ArtHeist['cuting'] then
                                    ...
                                    Framework.Functions.Notify(Strings['already_cuting'])
                                end
                            else
                                Framework.Functions.Notify('You dont armed a switchblade')
                            end
                        end
                    end
                end
...
AddEventHandler('artheist:client:policeAlert', function(targetCoords)
    Framework.Functions.Notify(Strings['police_alert'])
    local alpha = 250
...
function StartHeist()
    Framework.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
        if time then
            if ArtHeist['start'] then Framework.Functions.Notify(Strings['already_heist']) return end
            ...
            Framework.Functions.Notify(Strings['go_steal'])
...
function HeistAnimation(sceneId)
    ...
    if ArtHeist['cut'] == #Config['ArtHeist']['painting'] then
        ...
        Framework.Functions.Notify(Strings['go_sell'])
        RemoveBlip(stealBlip)
