    ...
    Framework.Functions.TriggerCallback('trainheist:server:checkPoliceCount', function(status)
        if status then
            Framework.Functions.TriggerCallback('trainheist:server:checkTime', function(time)
                if time then
                    Framework.Functions.Notify(Strings['goto_ambush'])
...
function FinishHeist()
    ...
    Framework.Functions.Notify(Strings['deliver_to_buyer'])
    loadModel('baller')
...
function OpenContainer(index)
    Framework.Functions.TriggerCallback('trainheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            local ped = PlayerPedId()
            local pedCo = GetEntityCoords(ped)
...
function Grab(index, index2)
    Framework.Functions.TriggerCallback('trainheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            -- ShowNotification(Strings['need_this'] .. itemLabel)
            Framework.Functions.Notify(Strings['need_this'] .. itemLabel)
        end
...
AddEventHandler('trainheist:client:showNotification', function(str)
    ...
    Framework.Functions.Notify(str)
end)
