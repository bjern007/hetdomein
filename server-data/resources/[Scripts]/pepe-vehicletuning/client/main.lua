...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Beunschuur Reparaties")
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Verberg het voertuig')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Pak het voertuig')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        VehicleList()
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Ga uit dienst")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Ga in dienst")
                        end
                        if IsControlJustReleased(0, Keys["E"]) then
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Plaats het voertuig op het platform")
                                            ...
                                            Framework.Functions.Notify("Je kunt geen fietsen op het platform zetten!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Open opties")
                                if IsControlJustPressed(0, Keys["E"]) then
                                    OpenMenu()
                                    Menu.hidden = not Menu.hidden
                                end
...
function VehicleOptions()
    ...
    Menu.addButton("Onderdelen menu", "PartsMenu", nil)
    Menu.addButton("Menu Sluiten", "CloseMenu", nil)
end
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repareren ("..Framework.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Terug", "VehicleOptions", nil)
    Menu.addButton("Menu Sluiten", "CloseMenu", nil) 
end
            ...
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    Framework.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        Framework.Functions.Notify("Reparatie geannuleerd.", "error")
                    end)
                    break
                else
                    Framework.Functions.Notify('Er zijn niet genoeg materialen in de kluis.', 'error')
                end
                break
        --  else
...
AddEventHandler('pepe-vehicletuning:client:RepaireeePart', function(part)
    ...
    Framework.Functions.Notify("De "..Config.ValuesLabels[part].." is gerepareerd!")
end)
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        Framework.Functions.Notify("Geen status bekend.", "error")
                    end
                else
                    Framework.Functions.Notify("Geen geldig voertuig.", "error")
                end
            else
                Framework.Functions.Notify("Je bent niet dicht genoeg bij het voertuig.", "error")
            end
        else
            Framework.Functions.Notify("Je moet eerst in het voertuig zijn.", "error")
        end
    else
        Framework.Functions.Notify("Je moet zich buiten het voertuig bevinden.", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            Framework.Functions.Notify("Je bent geen bestuurder Of je bent op de fiets.", "error")
        end
    else
        Framework.Functions.Notify("Je bevindt zich niet in een voertuig.", "error")
    end
end)
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            Framework.Functions.Progressbar("repair_advanced", "Repareren...", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            ...
                            Framework.Functions.Notify("Geen geldig onderdeel.", "error")
                        end
                    else
                        Framework.Functions.Notify("Geen geldig voertuig.", "error")
                    end
