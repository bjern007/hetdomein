...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    ...
    Framework.Functions.TriggerCallback('pepe-board:server:GetConfig', function(config)
    ...
    while true do
        if IsControlJustPressed(0, Config.OpenKey) then
            if not scoreboardOpen then
                Framework.Functions.TriggerCallback('pepe-board:server:GetPlayersArrays', function(playerList)
                    Framework.Functions.TriggerCallback('kwk-scoreboard:server:GetActivity', function(cops, ambulance, autocare, cardealers)
...
RegisterNetEvent('pepe-board:client:SetActivityBusy')
AddEventHandler('pepe-board:client:SetActivityBusy', function(activity, busy)
