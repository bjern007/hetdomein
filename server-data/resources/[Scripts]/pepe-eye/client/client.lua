...
function SendCurrentObjectData()
    ...
    elseif CurrentEntity['IsPlayer'] then
            SendNUIMessage({
                action = 'SetData',
                specific = false,
                currentdata = Config.PlayerMenu,
            })
            HitData = true
            ...
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        ...
                        Framework.Functions.Notify('Zelf ooit in een kofferbak van een motorfiets gehopt?', 'error', 2500)
                    end
                else
                    Framework.Functions.Notify('Is de kofferbak open?', 'error', 2500)
                end
            else
...
RegisterNetEvent('pepe-eye:client:open:memory')
AddEventHandler('pepe-eye:client:open:memory', function()
    Framework.Functions.Progressbar("mem-start", "Starten..", 1200, false, true, {
...
RegisterNetEvent('pepe-eye:client:open:optellen')
AddEventHandler('pepe-eye:client:open:optellen', function()
    Framework.Functions.Progressbar("optellen-start", "Starten..", 1200, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    TriggerEvent('pepe:numbers:start', 65, function(result)
...
AddEventHandler('pepe-eye:client:refresh', function()
    Framework.Functions.Progressbar("reset-eye", "Oog wordt gereset..", 1200, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
Citizen.CreateThread(function()
    while true do
		Citizen.Wait(0)
        local Distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 108.6601, -1287.105, 28.454256, true)
        if Distance <= 1.3 then
            if IsControlJustReleased(0, 38) and not FishyDEV then
			    FishyDEV = true
                LoadDict('mini@strip_club@pole_dance@pole_dance1')
                local scene = NetworkCreateSynchronisedScene(108.6601, -1287.105, 28.454256, vector3(0.0, 0.0, 0.0), 2, false, false, 1065353216, 0, 1.3)
