...
Citizen.CreateThread(function()
	...
	DrawText3D2(procesX, procesY, procesZ+0.1, "[E] Snijd hout")
		...
		Framework.Functions.Notify('Je hebt geen planken.', 'error')
		end
		end	
	end
...
function SellKurczaki22()
    ...
    if(count == 0) then
    Framework.Functions.Progressbar("search_register", "Hout snijden.", 5000, false, true, {disableMovement = true,
...
function SellKurczaki()
    ...
    ----
    if(count == 0) then
    Framework.Functions.Progressbar("search_register", "Verkopen..", 5000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
...
Citizen.CreateThread(function()
    while true do
        ...
        local dist = #(plyCoords - vector3(sellX4, sellY4, sellZ4)) 
        ...
        if dist <= 1.0 then
		    ...
		    if GetPedInVehicleSeat(vehicled, -1) == PlayerPedId() then
				...
				local s1 = false
                    Framework.Functions.Notify('Ik ben je auto aan het voorbereiden, 100% brandstof.', 'success',7000)
                    Framework.Functions.Progressbar("search_register", "Plaats de codekaart..", 7000, false, true, {
