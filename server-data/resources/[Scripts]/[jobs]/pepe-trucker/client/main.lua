                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        Framework.Functions.Notify('Dit is niet het bedrijfsvoertuig!', 'error')
                                    end
                                else
                                    Framework.Functions.Notify('Je moet bestuurder zijn..')
                                end
                            else
                                MenuGarage()
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                ...
                                Framework.Functions.TriggerCallback('pepe-trucker:01101110', function()
                                end, JobsDone)
                                JobsDone = 0
                                ...
                                Framework.Functions.Notify("Je hebt nog geen werk verricht..", "error")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, Keys["E"]) then
                                            isWorking = true
                                            Framework.Functions.Progressbar("work_carrybox", "Doos producten pakken..", 2000, false, true, {
                            ...
                            elseif hasBox then
                                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        Framework.Functions.Progressbar("work_dropbox", "Doos producten afleveren..", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                Framework.Functions.Notify("Je hebt alle producten afgeleverd, op naar het volgende punt")
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
                                                    CurrentBlip = nil
                                                end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        Framework.Functions.Notify("Je bent alle winkels langs gegaan.. Tijd voor je loonstrook!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
...
RegisterNetEvent('pepe-server:client:executeEvents')
AddEventHandler('pepe-server:client:executeEvents', function()
