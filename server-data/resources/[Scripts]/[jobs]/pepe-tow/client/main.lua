               ...
               if Config.CurrentNpc ~= nil then
                ...
                if Config.JobData[Framework.Functions.GetPlayerData().citizenid] ~= nil then
                  if Distance <= 2.0 and Config.JobData[Framework.Functions.GetPlayerData().citizenid]['Payment'] > 0 then
                     NearAnything = true
                     DrawText3D(Config.JobLocations['Laptop']['X'], Config.JobLocations['Laptop']['Y'], Config.JobLocations['Laptop']['Z'], '~g~E~s~ - Loonstrook')
                ...
                local Distance = GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.JobLocations['Flatbed']['X'], Config.JobLocations['Flatbed']['Y'], Config.JobLocations['Flatbed']['Z'], true)
                  if Distance <= 6.0 then
                   ...
                   if IsPedSittingInAnyVehicle(PlayerPedId()) then
                     DrawText3D(Config.JobLocations['Flatbed']['X'], Config.JobLocations['Flatbed']['Y'], Config.JobLocations['Flatbed']['Z'] + 0.2, '~g~E~s~ - Parkeren')
                     if IsControlJustReleased(0, 38) then
                     ...
                     DrawText3D(Config.JobLocations['Flatbed']['X'], Config.JobLocations['Flatbed']['Y'], Config.JobLocations['Flatbed']['Z'] + 0.2, '~g~E~s~ - Huren')
                     if IsControlJustReleased(0, 38) then
                       Framework.Functions.TriggerCallback("pepe-tow:server:do:bail", function(DidBail)
                           if DidBail then
                               GetTowVehicle()
                           else
                             Framework.Functions.Notify('Je hebt niet genoeg cash', 'error', 5500)
                           end
                        end)
                     end
                  end
...
AddEventHandler('pepe-tow:client:toggle:npc', function()
   if not Config.IsDoingNpc then
     ...
     Framework.Functions.Notify('Je bent begonnen met takelen', 'success', 5500)
   else
    if Config.CurrentTowedVehicle ~= nil then
        Framework.Functions.Notify('Je bent nog aan het afleveren of nog een open opdracht staan', 'error', 5500)
    else
        DeleteWaypoint()
        RemoveBlip(JobBlip)
        ResetAll()
        Framework.Functions.Notify('Je bent gestopt met het takelen', 'error', 5500)
    end
   end
end)
...
--             Framework.Functions.Progressbar("untowing_vehicle", "Removing Vehicle", 5000, false, true, {
...
--                     Framework.Functions.Notify("Voertuig verwijderd!", 'success')
...
--                 Framework.Functions.Notify("Voertuig verwijderd")
--             end, function() -- Cancel
--                 StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
        ...
        if Config.CurrentTowedVehicle == nil then
            if Distance <= 4.0 then
               if Vehicle ~= TowVehicle then
                       Framework.Functions.Progressbar("towing-vehicle", "Voertuig optakelen", 5000, false, true, {
                           disableMovement = true,
                           disableCarMovement = true,
                           disableMouse = false,
                           disableCombat = true,
                        ...
                        --    Framework.Functions.Notify("Vehicle attached, drive to Harold's Depot", "success")
                       end, function() -- Cancel
                           Config.CurrentTowedVehicle = nil
                           StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                           Framework.Functions.Notify("Gefaalt", "error")
                   ...
                   Framework.Functions.Notify('Ja dit lukt niet gap', 'error', 5500)
               end
            else
                Framework.Functions.Notify('Geen voertuig', 'error', 5500)
            end
        else
            Framework.Functions.Progressbar("untowing_vehicle", "Voertuig aftakelen", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
