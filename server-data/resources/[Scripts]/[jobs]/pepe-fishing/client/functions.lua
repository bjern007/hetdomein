TryToFish = function()
    Framework.Functions.TriggerCallback('pepe-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return Framework.Functions.Notify("Zwemmen en vissen tegelijk? Hmm interessant.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return Framework.Functions.Notify("Je kunt niet vissen in een auto.", "error") end 
        if count ~= nil then
            if count == 0 then
                Framework.Functions.Notify("Je hebt een Vishengel en aas nodig.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    Framework.Functions.Notify("Kijk naar 't water om te vissen", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        Framework.Functions.Notify('Je hebt geen aas!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        ShowHelpNotification("Werp je hengel met ~INPUT_DETONATE~")

        if GetGameTimer() - startedCasting > 5000 then
            Framework.Functions.Notify("Je moet het aas uitwerpen.", "primary")
    ...
    Framework.Functions.Notify("Wachten tot een vis bijt...", "success", "10000")
    ...
    if caughtFish then
        TriggerServerEvent("pepe-fishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('pepe-hud:Server:RelieveStress', 1)
    else
        Framework.Functions.Notify("De vis is losgeraakt.", "error")
    end
...
IsInWater = function()
    ...
    Framework.Functions.Notify("Vislocatie controleren...", "success", "3000")
...
SellFish = function()
    Framework.Functions.TriggerCallback('pepe-fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("pepe-fishing:sellFish", function(sold, fishesSold) end)
    end)
end
