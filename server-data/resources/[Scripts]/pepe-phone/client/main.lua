...
RegisterNetEvent('pepe-phone:client:addAmbulanceAlert')
AddEventHandler('pepe-phone:client:addAmbulanceAlert', function(alertData)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('pepe-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('pepe-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('pepe-lapraces:server:StartRace', data.RaceData.RaceId)
end)


RegisterNetEvent('pepe-phone:client:UpdateLapraces')
AddEventHandler('pepe-phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetRaces', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('pepe-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['pepe-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['pepe-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('pepe-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('pepe-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            Framework.Functions.Notify('Je bent te ver van de race. Je navigatie is ingesteld naar de race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['pepe-lapraces']:IsInEditor())
    else
        cb(exports['pepe-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)
        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    Framework.Functions.Notify("Actie momenteel niet mogelijk..", "error")
                end
            end
        end
        Citizen.Wait(3)
...
AddEventHandler('pepe-phone:client:send:email:bought:vehicle', function(Plate)
    ...
    SetTimeout(math.random(4500, 8000), function()
        TriggerServerEvent('pepe-phone:server:sendNewMail', {
            ...
            message = "Beste " .. Player.charinfo.firstname .. " " .. Player.charinfo.lastname .. ",<br /><br />Je hebt een nieuw voertuig met het kenteken: <b>" .. Plate .. "</b> aangekocht, veel veilige kilometers gewenst!<br /><br />Met vriendelijke groet,<br />Autoscout24",
            button = {}
        })
    end)
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        SendNUIMessage({
            ...
            Chats = PhoneData.Chats[ChatKey].messages,
        })
        SendNUIMessage({ 
            action = "RefreshAppAlerts",
            AppData = Config.PhoneApplications
...
AddEventHandler('pepe-phone:client:TransferMoney', function(amount, newmoney)
    ...
    if PhoneData.isOpen then
        ...
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Bank", content = "Er is €"..amount.." bijgeschreven!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
RegisterNUICallback('CallContact', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-phone:server:GetCallState', function(CanCall, IsOnline)
        ...
        cb(status)
            if CanCall and not status.InCall and (data.ContactData.number ~= PhoneData.PlayerData.charinfo.phone) and (data.ContactData.name ~= "Anoniem") then
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('pepe-phone:server:AddRecentCall', "missed", CallData, AnonymousCall)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        -- exports['pma-voice']:addPlayerToCall(PhoneData.CallData.CallId, 'Telefoon')
    else
        PhoneData.CallData.InCall = false
        PhoneData.CallData.CallType = nil
        PhoneData.CallData.AnsweredCall = false
...
RegisterNetEvent('pepe-phone:client:addAmbuAlert')
AddEventHandler('pepe-phone:client:addAmbuAlert', function(alertData)
    if PlayerJob.name == 'ambulance' and PlayerJob.onduty then
        SendNUIMessage({
            action = "AddAmbulanceAlert",
            alert = alertData,
...
RegisterNUICallback('applyJob', function(data)
    TriggerServerEvent('pepe-phone:server:ApplyJob', data.job, data.distance)
end)