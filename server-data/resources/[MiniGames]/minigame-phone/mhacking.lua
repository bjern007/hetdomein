	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navigeer met ~y~W,A,S,D~s~ en bevestig met ~y~SPACE~s~ voor de linker codeblok.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Gebruik de ~y~Pijltoetsen~s~ en bevestig met ~y~ENTER~s~ voor de rechter codeblok")
			else
