...
function BuildLocations(data)
    ...
    if data then
        if data.position and not data.jail then
            ...
            results[1] = { name = "last", label = "Laatste locatie", coords = coords, rot = rot, spawn = vector4(data.position.x, data.position.y, data.position.z, 100) };
        end
...
AddEventHandler("nvd-select:set" , function()
    BuildSelector(Framework.Functions.GetPlayerData());
end)